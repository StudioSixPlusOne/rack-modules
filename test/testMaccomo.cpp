...
using MA = MaccomoComp<TestComposite>;
...
static void testExtreme (float sr)
{
    ...
    ExtremeTester<MA>::test (ma, paramLimits, true, "Maccomo");
}
...
static void testSelfOscillate (const float voct, const float sr)
{
    ...
    ma.params[MA::RESONANCE_PARAM].setValue (9.0f);
    ma.params[MA::DRIVE_PARAM].setValue (1.5f);
    ma.params[MA::MODE_PARAM].setValue (1.0f);
...
static void testSelfOscillate()
{
    ...
    testSelfOscillate (-1.0f, 44100);
    testSelfOscillate (-2.0f, 5000);
