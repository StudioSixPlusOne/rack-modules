...
using MA = AmburghComp<TestComposite>;
...
static void testExtreme (float sr)
{
   ...
   ExtremeTester<MA>::test (ma, paramLimits, true, "Amburgh");
}
...
static void testSelfOscillate (const float voct, const float sr)
{
   ...
   ma.params[MA::RESONANCE_PARAM].setValue (4.0f);
   ma.params[MA::DRIVE_PARAM].setValue (6.0);
